...
class GroqDFIRClient:
    ...
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None):
        ...
        self.total_cost = 0.0
        self.api_calls = 0
        self.max_total_tokens = int(os.environ.get("GROQ_MAX_TOKENS", str(MAX_TOTAL_TOKENS_PER_SESSION)))
    ...
    def _check_token_budget(self, estimated_tokens: int = 4096) -> bool:
        ...
        if self.total_tokens + estimated_tokens > self.max_total_tokens:
            logger.warning(
                ...
                self.total_tokens, estimated_tokens, self.max_total_tokens,
            )
            return False
        return True
        ...
        for model in models_to_try:
            try:
                ...
                if hasattr(response, "usage") and response.usage:
                    ...
                    self.total_cost += (
                        ((usage.prompt_tokens or 0) / 1000) * COST_PER_1K_INPUT_TOKENS
                        + ((usage.completion_tokens or 0) / 1000) * COST_PER_1K_OUTPUT_TOKENS
                    )
                self.api_calls += 1

                logger.debug(
                    "Groq call (%s): tokens=%s, cost=$%.6f, calls=%s",
                    model, self.total_tokens, self.total_cost, self.api_calls,
                )
...
def _generate_narrative_report(all_findings: list, tool_calls: list) -> str:
    ...
    lines.append(f"**Mode:** Deterministic (no LLM)")
    lines.append("")
    ...
    if failed:
        ...
        for t in tool_calls:
            if isinstance(t, dict) and not t.get("success"):
                lines.append(f"- `{t.get('tool', 'unknown')}` failed: {t.get('error', 'unknown error')}")
        lines.append("")

    lines.append("## Recommendations")
    lines.append("")
    if successful > 0:
        lines.append("- Review findings from successful tool executions for indicators of compromise.")
    if failed > 0:
        lines.append("- Investigate tool failures — they may indicate corrupted evidence or missing dependencies.")
