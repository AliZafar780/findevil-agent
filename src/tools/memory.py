...
def analyze(
    memory_path: str, plugin: str = "linux.pslist.PsList", use_fallback: bool = True
) -> MemoryResult:
        ...
        if use_fallback:
            ...
            if findings:
                ...
                return MemoryResult(
                    plugin=f"{plugin}_fallback",
                    data=[
                        {"note": "No IOCs found via string scanning.", "plugin_attempted": plugin}
                    ],
                )

        return MemoryResult(
            success=False, plugin=plugin, error="Volatility 3 plugin returned no usable output"
        )
